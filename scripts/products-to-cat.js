...
async function prodToCat() {
    try {
        ...
        for(let i = 0; i<products.length; i++) {
            const x = await Category.findOne({"name":products[i].type, "company": products[i].addedCompany})
            if(!x) {
                const newCat = new Category({"name":products[i].type, "company": products[i].addedCompany})
                await newCat.save()
        ...
        for(let i = 0; i<products.length; i++) {
            for(let j=0; j<categories.length; j++) {
                if(products[i].addedCompany.equals(categories[j].company) && products[i].type === categories[j].name) {
                    if(!categories[j].products.includes(products[i]._id)) {
                        categories[j].products.push(products[i]._id)
                        await categories[j].save()
                        console.log(`added product ${products[i]._id} to ${categories[j].name} of company ${categories[j].company}`)
                    }
                    break;
                }
            }
